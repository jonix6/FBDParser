...
from ._global import _r, length, lines, bisize, color
...
class InlinePatterns:
    # 撑满注解（CM）
    CM_prefix = rf'(?P<cd>{_r(length)})'  # 长度
    CM_infix = rf'{CM_prefix}-(?P<zs>\d+)'  # 字/词数

    # 对齐注解（DQ）
    DQ_prefix = CM_prefix + '?'

    # 段首注解（DS）
    DS_infix = DS_prefix = rf'''
        (?P<gd>{_r(lines)})。  # 高度
        (?P<kd>{_r(length)})  # 宽度
        {border}?  # 边框说明
        (?:{background})?  # 底纹说明
    ...
    # 空格注解（KG）
    KG_prefix = rf'(?P<kg>-?{_r(length)})'  # 空格长度
    KG_infix = rf'{KG_prefix}(?P<zs>。\d*)?'  # 空格数
