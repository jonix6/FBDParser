...
from ._global import _r, lines, length, fontset, color
...
class ParagraphPatterns:
    ...
    # 改宽注解（GK）
    # 前后注解（QH）
    GK_infix = QH_infix = QH_prefix = rf'''(?:
        (?P<zy>-?{_r(length)}!?)  # 左右缩进
        |(?P<zs>-?{_r(length)})?  # 左缩进
        !(?P<ys>-?{_r(length)})  # 右缩进
    )'''
    ...
    # 行移注解（HY）
    HY_infix = rf'(?P<hy>D|{_r(length)})?'  # 行移参数：D行移距离取缺省距离，即：字高+行距
    ...
    # 居中注解（JZ）
    JZ_infix = rf'(?P<zj>{_r(length)})?'  # 字间距
    JZ_prefix = rf'(?:{JZ_infix}|{JY_prefix})?'
